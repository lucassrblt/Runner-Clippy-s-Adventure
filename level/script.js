...
function autoScroll() {
    ...
    hitDetection()
    if (obstaclesContainer.lastChild.getBoundingClientRect().left > 900 && gameActive == true) {
...
function hitDetection() {
    ...
    blocsB.forEach(blocB => {
        if (blocB.getBoundingClientRect().left > 205 && blocB.getBoundingClientRect().left < 265) {
            if (char.getBoundingClientRect().top > 268) {
                gameActive = false;
                losingScreen();
            }
        }
    ...
    blocsC.forEach(blocC => {
        if (blocC.getBoundingClientRect().left > 205 && blocC.getBoundingClientRect().left < 233) {
            if (char.getBoundingClientRect().top < 370) {
                gameActive = false;
                losingScreen();
            }
        }
